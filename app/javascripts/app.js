...
const ipfs = ipfsapi(current_ip, '5002');
...
function launchDeris(){
  ...
  //   if (err) throw err;
  // });
  window.open('http://localhost:3000/?id='+ elem('owner').innerHTML.toString(),'_self');//_blank to open in new tab
}
...
async function setAttributes(is_signup) {
  var elements = getElementsFromForm();
  profile.uuid = uuid;
  ...
  ipfs.files.add(Buffer.from(JSON.stringify(profile)), function (err, result) {
    if(!hasError(err)) {
  ...
  ipfs.files.cat(hash, function (err, stream) {
    if(!hasError(err)) {
      ...
      stream.on('data', function(buffer){
        file += JSON.stringify(buffer);
      });
      stream.on('end', function() {
        // Update attributes form
        //console.log("getting attr");
        //console.log(file)
        //console.log(typeof JSON.parse(JSON.stringify(file)));
        //console.log(JSON.parse(file).uuid);
        //console.log(JSON.parse(JSON.stringify(file)).uuid);
...
function deployContract(contract_name, callback) {
  ...
  // Get gas estimation
  web3.eth.estimateGas({data: "0x"+compiled_contract.bytecode}, function(err, gasEstimate) {
    log(contract_name + " contract gas estimate: " + gasEstimate);
    // Deploy contract
    if(!hasError(err))
      contracts[contract_name.toLowerCase()] = contract_obj.new(
        {from: address, data: "0x"+compiled_contract.bytecode, gas: gasEstimate},
...
window.addEventListener('load', function() {
  ...
  //log("accounts:"+web3.eth.accounts);
  // Get address balance
  web3.eth.getBalance(address, function(err, result){
    if(!hasError(err)) {
      
      console.log("Current balance:"+web3.fromWei(result, 'ether') + ' ether')
